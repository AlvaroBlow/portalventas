...
if($errores==0){
	//a crear usuario
	$sql="INSERT INTO `USUARIOS` VALUES ('".$email_s."', '".$password_s."', '', '-', '-', '-', '2');";	    	
	$conn->query($sql);	
	$creado=true;
...
function Continuar(tipo){
	...
	if(tipo==1){
	window.parent.location.href="KAM.php";
	}else{
	window.parent.location.href="usuario_crearKAM.php";		
	}
